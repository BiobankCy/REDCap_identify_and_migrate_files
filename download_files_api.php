...
#### The purpose of this script is to download all files in a project by identifying which fields are file type and have files stored (includes repeated measurements)
...
for($i=0;$i<$fieldnamesnum;$i++){
	...
	$data = array(
		...
		'type' => 'flat',
		'csvDelimiter' => '',
		'fields' => array('participant_id',$fieldnames[$i]),
		'rawOrLabel' => 'raw',
		...
		if(isset($names[$n]["redcap_repeat_instance"])){
			...
			if(trim($names[$n]["redcap_repeat_instance"])==NULL && trim($names[$n][$fieldnames[$i]])!=NULL){
				...
				if($output!="{\"error\":\"There is no file to download for this record\"}"){
					$outname=$names[$n]["participant_id"]."___".$fieldnames[$i]."___".$names[$n][$fieldnames[$i]];
				...
				if($output!="{\"error\":\"There is no file to download for this record\"}"){
					$outname=$names[$n]["participant_id"]."___".$names[$n]["redcap_repeat_instance"]."___".$fieldnames[$i]."___".$names[$n][$fieldnames[$i]];
					$fp = fopen("files/$outname", 'w');
					fwrite($fp, $output);
					fclose($fp);
					echo "Downloaded $outname\n";
